...
export function BookingModal({ 
  ...
  const getAvailableCurrencies = () => {
    const list = new Set(['BRL'])
    if (therapist.services) {
      therapist.services.forEach(s => {
        if (s.priceUsd != null) list.add('USD')
        if (s.priceEur != null) list.add('EUR')
        s.packages?.forEach(p => {
          if (p.priceUsd != null) list.add('USD')
          if (p.priceEur != null) list.add('EUR')
        })
      })
    }
  ...
  const getServiceDisplayPrice = (svc: TherapistService) => {
    if (selectedCurrency === 'USD' && svc.priceUsd != null) {
      const charge = effectiveServiceCharge({ price: Number(svc.priceUsd), promoPrice: svc.promoPriceUsd ? Number(svc.promoPriceUsd) : null })
      return { price: charge, currency: 'USD' }
    }
    if (selectedCurrency === 'EUR' && svc.priceEur != null) {
      const charge = effectiveServiceCharge({ price: Number(svc.priceEur), promoPrice: svc.promoPriceEur ? Number(svc.promoPriceEur) : null })
      return { price: charge, currency: 'EUR' }
    }
    const charge = effectiveServiceCharge({ price: Number(svc.price), promoPrice: svc.promoPrice ? Number(svc.promoPrice) : null })
    return { price: charge, currency: 'BRL' }
  }

  const getPackageDisplayPrice = (pkg: TherapyPackage) => {
    if (selectedCurrency === 'USD' && pkg.priceUsd != null) {
      return { price: Number(pkg.priceUsd), currency: 'USD' }
    }
    if (selectedCurrency === 'EUR' && pkg.priceEur != null) {
      return { price: Number(pkg.priceEur), currency: 'EUR' }
    }
    return { price: Number(pkg.price), currency: 'BRL' }
  }

  const getSelectedPriceAndCurrency = () => {
    if (usePackageId) {
      return { price: 0, currency: 'BRL' }
    }
    if (selectedPackage) {
      return getPackageDisplayPrice(selectedPackage)
    }
    if (selectedService) {
      return getServiceDisplayPrice(selectedService)
    }
    return { price: parsePrice(therapist.price), currency: 'BRL' }
  }

  const { price, currency } = getSelectedPriceAndCurrency()

  const hasPackages = selectedService && selectedService.packages && selectedService.packages.length > 0
  
  const availableCurrencies = getAvailableCurrencies()
  const showCurrencyStep = availableCurrencies.length > 1 && !usePackageId

  const baseSteps = hasServices 
    ? (hasPackages 
        ? ['service', 'plan', 'date', 'time', 'payment', 'confirm'] 
        : ['service', 'date', 'time', 'payment', 'confirm'])
    : ['date', 'time', 'payment', 'confirm']

  const stepsWithCurrency = showCurrencyStep ? ['currency', ...baseSteps] : baseSteps
  const effectiveSteps = usePackageId ? stepsWithCurrency.filter(s => s !== 'payment') : stepsWithCurrency

  const [step, setStep] = useState<string>(() => {
    if (showCurrencyStep) return 'currency'
    return hasServices ? 'service' : 'date'
  })
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const availCurs = getAvailableCurrencies()
      const hasMult = availCurs.length > 1 && !initialUsePackageId
      ...
      if (initialUsePackageId) {
        ...
        if (svc) {
          ...
          setStep(hasMult ? 'currency' : 'date')
        }
      } else {
        setStep(hasMult ? 'currency' : (hasServices ? 'service' : 'date'))
      }
    }
  }, [isOpen, initialServiceId, initialPackageId, initialUsePackageId, therapist.services, showCurrencyStep])
  ...
  // Removed duplicate parsePrice to top level
  ...
  const goNext = async () => {
    ...
    if (idx < effectiveSteps.length - 1) {
      if (step === 'currency') {
        if (selectedPackage) {
          const hasCurrencyPrice = selectedCurrency === 'USD' ? selectedPackage.priceUsd != null : selectedCurrency === 'EUR' ? selectedPackage.priceEur != null : true
          if (hasCurrencyPrice) {
            setStep('date')
            return
          } else {
          ...
          const hasCurrencyPrice = selectedCurrency === 'USD' ? selectedService.priceUsd != null : selectedCurrency === 'EUR' ? selectedService.priceEur != null : true
          if (hasCurrencyPrice) {
            const pkgCount = (selectedService.packages || []).filter(p => {
              if (selectedCurrency === 'USD') return p.priceUsd != null
              if (selectedCurrency === 'EUR') return p.priceEur != null
              return true
            }).length
            if (pkgCount > 0) {
              setStep('plan')
    ...
    <Modal isOpen={isOpen} onClose={handleClose} title="Agendar sessão" size="lg">
      {step === 'done' ? (
        ...
        <div className="space-y-6">
          ...
          {/* STEP: Selecionar Moeda */}
          {step === 'currency' && showCurrencyStep && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-5">
              <div>
                <h4 className="font-extrabold text-slate-900 text-base">Moeda de pagamento</h4>
                <p className="text-xs text-slate-500 mt-1">
                  Selecione em qual moeda deseja visualizar os preços e realizar o pagamento de sua consulta.
                </p>
              </div>

              <div className="space-y-3">
                {availableCurrencies.map((cur) => {
                  const label = cur === 'BRL' ? 'Real (BRL)' : cur === 'USD' ? 'Dólar (USD)' : 'Euro (EUR)'
                  const symbol = cur === 'BRL' ? 'R$' : cur === 'USD' ? '$' : '€'
                  return (
                    <button
                      key={cur}
                      type="button"
                      ...
                      className={cn(
                        ...
                        selectedCurrency === cur
                          ? "border-[#0090FF] bg-blue-50/40 shadow-sm"
                          : "border-slate-100 bg-white hover:border-slate-200"
                      )}
                    >
                      <div>
                        <p className="font-bold text-slate-800 text-xs">{label}</p>
                        <p className="text-[10px] text-slate-400 mt-0.5">Pagamento em {cur}</p>
                      </div>
                      <span className="text-base font-black text-[#0090FF]">{symbol}</span>
                    </button>
                  )
                })}
              </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="animate-in fade-in slide-in-from-top-4 duration-500">
                       ...
                       <div className="space-y-3">
                         {therapist.services
                            ...
                            .filter(pkg => {
                              if (selectedCurrency === 'USD') return pkg.priceUsd != null
                              if (selectedCurrency === 'EUR') return pkg.priceEur != null
                              return true
                            })
                            .map(pkg => (
                             <button
                                ...
                                <div className="flex justify-between items-center gap-3 relative z-10">
                                   <div>
                                    ...
                                    <div className="text-right">
                                       <p className="font-black text-[#C5A03F] text-xl">
                                         {(() => {
                                           const display = getPackageDisplayPrice(pkg);
                                           return formatCurrency(display.price, display.currency);
                                         })()}
                                       </p>
                  ...
                  <div className="pt-2 border-t border-slate-100">
                    ...
                    <div className="space-y-3">
                      {therapist.services!
                        .filter(svc => {
                          if (selectedCurrency === 'USD') return svc.priceUsd != null
                          if (selectedCurrency === 'EUR') return svc.priceEur != null
                          return true
                        })
                        .map((svc) => (
                        <button
                          ...
                          <div className="flex justify-between items-start gap-3">
                            <div className="flex-1">
                             ...
                             <p className="font-extrabold text-[#0090FF] text-lg">
                               {(() => {
                                 const display = getServiceDisplayPrice(svc);
                                 return formatCurrency(display.price, display.currency);
                               })()}
                             </p>
                           </div>
