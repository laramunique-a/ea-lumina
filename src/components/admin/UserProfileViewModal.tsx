...
import { 
  ...
  User, Mail, Phone, Calendar, MapPin, Briefcase, Award, Globe, Video, Link as LinkIcon
} from 'lucide-react'
...
export function UserProfileViewModal({
  ...
  const renderField = (
    ...
    const isMissing = completeness.missingMandatory.some(m => m.field === fieldName) || 
    ...
    return (
      ...
      >
        ...
        <div className={`text-sm font-medium ${isMissing ? 'text-slate-400 italic' : 'text-slate-800'}`}>
          {isMissing ? 'Não preenchido' : String(value)}
        </div>
      </div>
    )
  }
  ...
  return (
    ...
    >
      <div className="space-y-6 max-h-[80vh] overflow-y-auto pr-1">
        ...
        <div className="flex flex-col md:flex-row items-center gap-5 p-5 bg-slate-50 rounded-3xl border border-slate-100/60">
          <div className="relative">
            <Image
              ...
              width={72}
              height={72}
              className="w-18 h-18 rounded-2xl border-2 border-white shadow-sm object-cover"
            />
          </div>
        ...
        {/* Grid de Seções de Campos */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {/* Seção 1: Dados Gerais */}
          <div className="space-y-4">
            <h4 className="text-sm font-black text-slate-800 tracking-wider uppercase flex items-center gap-2 px-1">
            ...
            <div className="grid grid-cols-1 gap-3">
              {role === 'TERAPEUTA' && tp && (
                <>
                  ...
                  {(tp.modality === 'PRESENCIAL' || tp.modality === 'AMBOS') && (
                    <>
                      {(() => {
              ...
              {role === 'ADMIN' && (
                <div className="p-4 rounded-xl border border-slate-100 bg-slate-50/30 text-center text-xs text-slate-400">
                  Nenhum campo de endereço cadastrado para administradores.
                </div>
              )}
            </div>
          </div>
        ...
        {/* Seção 3: Informações Profissionais ou Anamnese */}
        {role === 'TERAPEUTA' && tp && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                {renderField('Biografia', tp.bio, 'bio', true)}
                {renderField('Valor de Sessão (Preset)', tp.price ? `R$ ${Number(tp.price).toFixed(2)}` : '', 'price', false)}
                {renderField('Anos de Experiência', tp.yearsExp, 'yearsExp', false)}
                {renderField('Especialidades', tp.therapies?.join(', '), 'therapies', false)}
              </div>
            </div>
        ...
        {/* Seção 4: Documentos de Validação e Certificados */}
        {role === 'TERAPEUTA' && tp && (
          <div className="space-y-4 pt-4 border-t border-slate-100">
            <h4 className="text-sm font-black text-slate-800 tracking-wider uppercase flex items-center gap-2 px-1">
              <Award size={16} className="text-[#0090FF]" />
              Documentos e Certificações para Validação
            </h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-5 rounded-2xl border border-slate-100 bg-slate-50/50">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-bold text-slate-700 font-sans">Certificações e Diplomas</span>
