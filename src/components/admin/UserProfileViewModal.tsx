...
import { getAvatarUrl, formatCurrency } from '@/lib/utils'
import { getProfileCompleteness } from '@/lib/profile-completeness'
import { 
  X, AlertCircle, CheckCircle2, FileText, Download, Eye, 
  User, MapPin, Briefcase, Award, Globe, Layers
} from 'lucide-react'
      ...
      <div className="space-y-6 max-h-[80vh] overflow-y-auto pr-1">
          ...
          <div className="divide-y divide-slate-100">
            {role === 'TERAPEUTA' && tp ? (
              ...
              <>
                ...
                {renderFieldRow('Profissão', pp.profession, 'profession', true)}
              </>
            ) : (
              // ADMIN
              <>
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-3">
            ...
            <div className="divide-y divide-slate-100">
              {role === 'TERAPEUTA' && tp ? (
                ...
                // PACIENTE
                <>
                  {renderFieldRow('CEP', pp?.zipCode, 'zipCode', true)}
                  {renderFieldRow('Cidade', pp?.city, 'city', true)}
                  {renderFieldRow('Estado / Província', pp?.state, 'state', true)}
                  {renderFieldRow('País', pp?.country, 'country', true)}
                  {renderFieldRow('Bairro', pp?.neighborhood, 'neighborhood', true)}
                  {renderFieldRow('Rua', pp?.street, 'street', true)}
                  {renderFieldRow('Número', pp?.number, 'number', true)}
        ...
        {role === 'PACIENTE' && pp && (
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-3">
            ...
            <div className="divide-y divide-slate-100">
              {renderFieldRow('Objetivo Terapêutico', pp.anamnese?.objetivo, 'anamnese.objetivo', true)}
              {renderFieldRow('Histórico Emocional', pp.anamnese?.historicoEmocional, 'anamnese.historicoEmocional', true)}
              {renderFieldRow('Expectativas', pp.anamnese?.expectativas, 'anamnese.expectativas', true)}
              {renderFieldRow('Uso de Medicamentos', pp.anamnese?.medicamentos, 'anamnese.medicamentos', true)}
              {renderFieldRow('Alergias / Restrições', pp.anamnese?.alergias, 'anamnese.alergias', true)}
            </div>
          </div>
        )}
        ...
        {/* SEÇÃO: TERAPIAS CADASTRADAS (VALORES) */}
        {role === 'TERAPEUTA' && tp && (
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-sm space-y-4">
            <h4 className="text-sm font-black text-slate-800 tracking-wider uppercase flex items-center gap-2 px-1 pb-2 border-b border-slate-100/80">
              <Layers size={16} className="text-[#0090FF]" />
              Terapias Cadastradas & Valores
            </h4>
            
            {tp.services && tp.services.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {tp.services.map((service: any) => (
                  <div 
                    key={service.id} 
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="min-w-0">
                        <h5 className="font-bold text-slate-800 text-sm truncate" title={service.name}>{service.name}</h5>
                        {service.description && (
                          <p className="text-xs text-slate-500 mt-1 line-clamp-2" title={service.description}>
                            {service.description}
                          </p>
                        )}
                      </div>
                      <Badge variant={service.active ? 'success' : 'default'} size="sm" className="shrink-0">
                        {service.active ? 'Ativo' : 'Inativo'}
                      </Badge>
                    </div>

                    <div className="flex flex-wrap items-center gap-2 text-[10px] uppercase font-bold text-slate-400 mb-3">
                      <span>{service.durationMinutes} min</span>
                      <span>•</span>
                      <span>{service.modality === 'AMBOS' ? 'Online/Presencial' : service.modality}</span>
                    </div>

                    <div className="bg-white rounded-xl p-3 border border-slate-100/80 space-y-1.5">
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-slate-400 font-medium">BRL (R$):</span>
                        <span className="font-bold text-slate-800">
                          {service.price != null ? formatCurrency(service.price, 'BRL') : '—'}
                          {service.promoPrice ? (
                            <span className="text-[10px] text-green-600 ml-1.5 font-semibold">
                              (Promo: {formatCurrency(service.promoPrice, 'BRL')})
                            </span>
                          ) : null}
                        </span>
                      </div>

                      {(service.priceUsd != null || service.promoPriceUsd != null) && (
                        <div className="flex justify-between items-center text-xs">
                          <span className="text-slate-400 font-medium">USD ($):</span>
                          <span className="font-bold text-slate-800">
                            {service.priceUsd != null ? formatCurrency(service.priceUsd, 'USD') : '—'}
                            {service.promoPriceUsd ? (
                              <span className="text-[10px] text-green-600 ml-1.5 font-semibold">
                                (Promo: {formatCurrency(service.promoPriceUsd, 'USD')})
                              </span>
                            ) : null}
                          </span>
                        </div>
                      ...
                      {(service.priceEur != null || service.promoPriceEur != null) && (
                        <div className="flex justify-between items-center text-xs">
                          <span className="text-slate-400 font-medium">EUR (€):</span>
                          <span className="font-bold text-slate-800">
                            {service.priceEur != null ? formatCurrency(service.priceEur, 'EUR') : '—'}
                            {service.promoPriceEur ? (
                              <span className="text-[10px] text-green-600 ml-1.5 font-semibold">
                                (Promo: {formatCurrency(service.promoPriceEur, 'EUR')})
                              </span>
                            ) : null}
                          </span>
                        </div>
              ...
              <p className="text-xs text-slate-400 italic px-1">Nenhuma terapia/serviço cadastrado por este terapeuta.</p>
            )}
          </div>
        )}
