...
export function OnboardingChecklist({ checklist }: OnboardingChecklistProps) {
  ...
  useEffect(() => {
    if (currentChecklist.allComplete) return

    const fetchChecklistStatus = async () => {
      try {
        const res = await fetch('/api/therapist/checklist')
        const data = await res.json()
        if (data.success && data.checklist) {
          setCurrentChecklist(data.checklist)
        }
      } catch (err) {
        console.error('Error fetching onboarding checklist status:', err)
      }
    }

    fetchChecklistStatus() // Fetch immediately on mount

    const interval = setInterval(fetchChecklistStatus, 4000)
    return () => clearInterval(interval)
  }, [currentChecklist.allComplete])

  if (currentChecklist.allComplete) return null
  ...
  const items = [
    {
      ...
      completed: currentChecklist.profileComplete,
    ...
    },
    {
      ...
      completed: currentChecklist.therapiesComplete,
