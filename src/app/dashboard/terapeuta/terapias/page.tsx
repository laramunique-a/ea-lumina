              ...
              >
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm text-slate-600 mt-1">
                    <span className="font-bold text-[#0090FF]">
                  ...
                  {t.packages && t.packages.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                       {t.packages.map(p => (
                         <span key={p.id} className="text-[10px] font-bold bg-blue-50 text-[#0090FF] px-2 py-1 rounded-lg border border-blue-100 uppercase tracking-tight">
                           {p.sessionCount}x {p.name}
                         </span>
                       ))}
                    </div>
                  )}
          ...
          <div className="space-y-4 pt-1">
            ...
            <div className="pt-4 border-t border-slate-100">
              <div className="flex items-center justify-between mb-4">
                <h5 className="text-sm font-bold text-slate-800 uppercase tracking-wide">Pacotes de Sessões</h5>
                <Button 
                  type="button" 
                  variant="secondary" 
                  size="sm" 
                  className="rounded-full shadow-sm"
                  onClick={() => setPackages([...packages, { name: 'Novo Pacote', sessionCount: 5, price: 0, active: true }])}
                >
                  <Plus size={16} className="text-[#0090FF]" />
                  <span className="text-[#0090FF]">Adicionar Pacote</span>
                </Button>
              </div>

              {packages.length === 0 ? (
                <p className="text-xs text-slate-400 italic bg-slate-50 p-4 rounded-xl border border-dashed border-slate-200 text-center">
                  Crie pacotes para incentivar o paciente a agendar mais vezes.
                </p>
              ) : (
                <div className="space-y-3">
                  {packages.map((pkg, idx) => (
                    <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-5 relative group transition-all hover:shadow-md">
                      <div className="flex items-start gap-4">
                        <div className="flex-1">
                          <Input 
                            ...
                            size="md"
                            value={pkg.name} 
                            placeholder="Ex: Combo 5 Sessões"
                        ...
                        <button 
                           ...
                           className="mt-8 p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-1">
                        <Input 
                          label="Qtd Sessões" 
                          type="number" 
                          value={pkg.sessionCount ?? 0} 
                          onChange={(e) => {
                            const newPkgs = [...packages];
                            newPkgs[idx].sessionCount = parseInt(e.target.value) || 0;
                            setPackages(newPkgs);
                          }}
                        />
                        <Input 
                        ...
                        <Input 
                          ...
                          placeholder="Ex: 180"
                          type="number" 
                          hint="Deixe vazio para vitalício"
                          value={pkg.expirationDays ?? ''} 
                          onChange={(e) => {
                            const newPkgs = [...packages];
                            newPkgs[idx].expirationDays = (e.target.value === '') ? null : parseInt(e.target.value);
                            setPackages(newPkgs);
                          }}
                        />
                      </div>

                      {/* Checkbox em Destaque na Base */}
                      <div className="p-4 bg-blue-50/40 rounded-2xl border border-blue-100/50">
                        <label className="flex items-start gap-3 cursor-pointer group/toggle">
                          <input 
                            type="checkbox"
                            checked={pkg.isMultiTherapy}
                            onChange={(e) => {
                              const newPkgs = [...packages];
                              newPkgs[idx].isMultiTherapy = e.target.checked;
                              setPackages(newPkgs);
                            }}
                            className="w-5 h-5 rounded border-slate-300 text-[#0090FF] focus:ring-[#0090FF]/30 mt-0.5"
                          />
                          <div className="flex-1">
                            <span className="text-xs font-black text-slate-800 uppercase tracking-widest block">Válido em todas as minhas terapias</span>
                            <span className="text-[10px] text-slate-500 font-medium leading-relaxed block mt-1">
                              Ative para que o paciente possa usar este pacote em qualquer serviço que você oferece.
                            </span>
                          </div>
                        </label>
                      </div>
