...
import { X, Plus, Save, Upload, FileText, ExternalLink, Trash2, User, Camera, Phone, CreditCard, Eye, Download, CheckCircle2, Clock, Video, MapPin } from 'lucide-react'
...
export default function TerapeutaPerfilPage() {
  ...
  const [addresses, setAddresses] = useState<{ id?: string; cep: string; street: string; number: string; complement: string; neighborhood: string; city: string; state: string; country: string }[]>([])
  ...
  const loadProfile = useCallback(async (opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      if (tp) {
        ...
        setCity(tp.city || '')
        setState(tp.state || '')
        setAddresses((tp as any).addresses || [])
  ...
  const handleSave = async (section: 'basic' | 'contact' | 'bio' | 'certificates' | 'all' = 'all') => {
    ...
    if (section === 'all' || section === 'contact') {
      ...
      if (modality !== 'ONLINE' && addresses.length === 0) {
        toast.error('Por favor, adicione pelo menos um endereço físico de atendimento.')
        return
      }
    }
    ...
    try {
      ...
      if (section === 'all' || section === 'contact') {
        ...
        profileBody.addresses = modality === 'ONLINE' ? [] : addresses.map(addr => ({
          cep: addr.cep,
          street: addr.street,
          number: addr.number,
        ...
        // Sincronizar localmente no payload de salvamento
        if (modality !== 'ONLINE' && addresses.length > 0) {
          const primary = addresses[0]
          profileBody.city = primary.city
          profileBody.state = primary.state.trim().toUpperCase()
          profileBody.country = primary.country || 'Brasil'
          ...
          profileBody.city = null
          profileBody.state = null
          profileBody.country = null
          profileBody.location = null
        }
            ...
            <div className="mt-6 border-t border-slate-100 pt-6 space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                  ...
                  Endereços de Atendimento Físico ({addresses.length})
                </h3>
              </div>
              ...
              {addresses.length > 0 ? (
                  ...
                  {addresses.map((addr, idx) => (
                    <div 
                      ...
                      className={`p-4 rounded-2xl border relative flex flex-col justify-between transition-all ${
                        idx === 0 
                          ? 'border-blue-100 bg-blue-50/10 shadow-[0_2px_8px_-3px_rgba(0,144,255,0.08)]' 
                    ...
                    >
                      <div className="pr-8 space-y-1">
                        <div className="flex items-center gap-1.5 mb-1.5">
                          <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full ${
                            idx === 0 
                              ? 'bg-blue-100 text-[#0090FF]' 
                              : 'bg-slate-100 text-slate-500'
                          }`}>
                            {idx === 0 ? 'Principal / Busca' : `Endereço ${idx + 1}`}
                          </span>
                        </div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          toast.success('Endereço removido (clique em Salvar Alterações para persistir)')
                        }}
                        className="absolute right-3 top-3 p-1.5 rounded-lg text-slate-400 hover:text-red-500 hover:bg-slate-50 transition-colors"
                ...
                <div className="text-center py-6 px-4 rounded-2xl border border-dashed border-slate-200 bg-slate-50/30">
                  <MapPin size={24} className="text-slate-300 mx-auto mb-2" />
                  <p className="text-xs font-semibold text-slate-400">Nenhum endereço físico cadastrado ainda.</p>
                  <p className="text-[10px] text-slate-400 mt-0.5">Adicione pelo menos um endereço abaixo.</p>
                </div>
              )}

              {/* Formulário para adicionar um novo endereço */}
              <div className="border-t border-slate-100/80 pt-6 space-y-4">
                <h4 className="text-xs font-black uppercase tracking-widest text-slate-400">Adicionar novo endereço</h4>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <Input
                    className="bg-slate-50 border-slate-100/50 rounded-xl"
                    label="CEP"
                    value={newCep}
                    onChange={(e) => setNewCep(cepMask(e.target.value))}
                    onBlur={async () => {
                ...
                <div className="flex justify-start">
                  <button
                    ...
                    onClick={() => {
                      if (!newStreet.trim()) {
                        toast.error('O logradouro/rua é obrigatório.')
                        return
                      }
                      if (!newCity.trim()) {
                      ...
                      setNewState('')
                      setNewCountry('Brasil')
                      toast.success('Endereço adicionado à lista!')
                    }}
