    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-surface-200 shadow-card overflow-hidden">
        ...
        <div className="p-6 space-y-6 border-t border-surface-100">
          ...
          {isPresencial && profile.addresses && profile.addresses.length > 0 && (
            <section>
              <h2 className="font-semibold text-slate-900 mb-3 flex items-center gap-2">
                <MapPin size={18} className="text-[#C5A03F]" />
                Locais de Atendimento Presencial
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {profile.addresses.map((addr: any, idx: number) => (
                  <div key={idx} className="p-4 rounded-2xl border border-slate-100 bg-slate-50/30 flex gap-3">
                    <MapPin size={20} className="text-[#C5A03F] shrink-0 mt-0.5" />
                  ...
                  {profile.services.flatMap(svc => svc.packages.map(pkg => ({ ...pkg, svcName: svc.name, svcId: svc.id }))).map(pkg => (
                    <div key={pkg.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all group flex flex-col justify-between">
                       <div>
                          ...
                          {(pkg.priceUsd != null || pkg.priceEur != null) && (
                             <div className="flex flex-col gap-0.5 mt-2 bg-slate-50 border border-slate-100/80 rounded-xl p-2.5 text-[10px] font-bold text-slate-500">
                               {pkg.priceUsd != null && (
                                 <div className="flex justify-between">
                                   <span>Total: $ {Number(pkg.priceUsd).toFixed(2)} USD</span>
                                   <span className="text-brand-600">($ {(Number(pkg.priceUsd) / pkg.sessionCount).toFixed(2)} / sessão)</span>
                                 </div>
                               )}
                               {pkg.priceEur != null && (
                                 <div className="flex justify-between">
                                   <span>Total: € {Number(pkg.priceEur).toFixed(2)} EUR</span>
                                   <span className="text-brand-600">(€ {(Number(pkg.priceEur) / pkg.sessionCount).toFixed(2)} / sessão)</span>
                                 </div>
                               )}
                             </div>
                           )}
                  ...
                  >
                    <div className="flex justify-between items-start gap-3">
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        {(svc.priceUsd != null || svc.priceEur != null) && (
                          <div className="flex flex-col items-end text-[10px] font-semibold text-slate-500 mt-1">
                            {svc.priceUsd != null && <span>$ {Number(svc.priceUsd).toFixed(2)} USD</span>}
                            {svc.priceEur != null && <span>€ {Number(svc.priceEur).toFixed(2)} EUR</span>}
                          </div>
                        )}
                      </div>
                    </div>
