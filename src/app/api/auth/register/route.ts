...
import { getWelcomePatientEmail, getWelcomeTherapistEmail } from '@/lib/email-templates'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if (!automation) {
        const tpl = role === 'TERAPEUTA'
          ? getWelcomeTherapistEmail(name)
          : getWelcomePatientEmail(name)
      ...
      if (automation && automation.active) {
        ...
        const personalizedSubject = automation.subject.includes('{{nome}}')
          ? automation.subject.replace(/\{\{nome\}\}/g, name || 'Usuário')
          : tpl.subject
        const personalizedContent = automation.content.includes('{{nome}}')
          ? automation.content.replace(/\{\{nome\}\}/g, name || 'Usuário')
          : tpl.html
