...
export async function POST(request: NextRequest) {
  try {
    ...
    // Regra: se já avaliou este terapeuta para este mesmo serviço (ou individual se null)
    const serviceId = appointment.serviceId
    const therapistId = appointment.therapistId

    const existingEvalForService = await prisma.review.findFirst({
      where: {
    ...
    if (existingEvalForService) {
      const therapyName = appointment.service?.name || 'Sessão Individual'
      return NextResponse.json({ 
        success: false, 
        error: `Você já avaliou o serviço "${therapyName}" deste terapeuta.` 
      }, { status: 400 })
    }
