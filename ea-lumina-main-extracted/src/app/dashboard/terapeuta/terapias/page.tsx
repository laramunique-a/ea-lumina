...
export default function TerapeutaTerapiasPage() {
  ...
  const [pkgPriceUsd, setPkgPriceUsd] = useState('')
  const [pkgPriceEur, setPkgPriceEur] = useState('')
  ...
  const openEditTherapy = (t: TherapyRow) => {
    ...
    setFormPrice(t.price != null ? formatBRL(String(t.price)) : '')
    setFormPriceUsd(t.priceUsd ? formatBRL(String(t.priceUsd)) : '')
    setFormPriceEur(t.priceEur ? formatBRL(String(t.priceEur)) : '')
  ...
  const handleSaveTherapy = async () => {
    ...
    const price = formPrice.trim() ? parseFloat(formPrice.replace(/\./g, '').replace(',', '.')) : null
    const priceUsd = formPriceUsd.trim() ? parseFloat(formPriceUsd.replace(/\./g, '').replace(',', '.')) : null
    const priceEur = formPriceEur.trim() ? parseFloat(formPriceEur.replace(/\./g, '').replace(',', '.')) : null
    const duration = parseInt(formDuration, 10)
    
    if (price !== null && (!Number.isFinite(price) || price <= 0)) return toast.error('Informe um valor válido maior que zero (R$).')
    if (priceUsd !== null && (!Number.isFinite(priceUsd) || priceUsd <= 0)) return toast.error('Valor em Dólar deve ser maior que zero.')
    if (priceEur !== null && (!Number.isFinite(priceEur) || priceEur <= 0)) return toast.error('Valor em Euro deve ser maior que zero.')
    if (!price && !priceUsd && !priceEur) return toast.error('Informe ao menos um valor (R$, US$ ou €) para o atendimento.')
  ...
  const openEditPackage = (parentServiceId: string, pkg: TherapyPackage) => {
    ...
    setPkgPrice(pkg.price != null ? formatBRL(String(pkg.price)) : '')
    setPkgPriceUsd(pkg.priceUsd ? formatBRL(String(pkg.priceUsd)) : '')
    setPkgPriceEur(pkg.priceEur ? formatBRL(String(pkg.priceEur)) : '')
  ...
  const handleSavePackage = async () => {
    ...
    const price = pkgPrice.trim() ? parseFloat(pkgPrice.replace(/\./g, '').replace(',', '.')) : null
    const priceUsd = pkgPriceUsd.trim() ? parseFloat(pkgPriceUsd.replace(/\./g, '').replace(',', '.')) : null
    const priceEur = pkgPriceEur.trim() ? parseFloat(pkgPriceEur.replace(/\./g, '').replace(',', '.')) : null
    ...
    if (price !== null && (!Number.isFinite(price) || price <= 0)) return toast.error('Valor total (R$) inválido.')
    if (priceUsd !== null && (!Number.isFinite(priceUsd) || priceUsd <= 0)) return toast.error('Valor em Dólar deve ser maior que zero.')
    if (priceEur !== null && (!Number.isFinite(priceEur) || priceEur <= 0)) return toast.error('Valor em Euro deve ser maior que zero.')
    if (!price && !priceUsd && !priceEur) return toast.error('Informe ao menos um valor (R$, US$ ou €) para o pacote.')
                      ...
                      >
                        <div className="flex-1">
                          ...
                          <div className="flex items-end gap-2 flex-wrap">
                            {t.price != null ? (
                              <span className="font-black text-[#0090FF] text-xl">R$ {t.price.toFixed(2)}</span>
                            ) : t.priceUsd != null ? (
                              <span className="font-black text-[#0090FF] text-xl">$ {t.priceUsd.toFixed(2)} USD</span>
                            ) : t.priceEur != null ? (
                              <span className="font-black text-[#0090FF] text-xl">€ {t.priceEur.toFixed(2)} EUR</span>
                            ) : null}
                            <span className="text-sm font-medium text-slate-400 mb-0.5">/ {t.durationMinutes} min</span>
                          </div>
                          {/* Moedas adicionais (que não a principal já exibida) */}
                          {(() => {
                            const extras = [];
                            if (t.price != null && t.priceUsd != null) extras.push(`$ ${t.priceUsd.toFixed(2)} USD`);
                            if (t.price != null && t.priceEur != null) extras.push(`€ ${t.priceEur.toFixed(2)} EUR`);
                            if (t.price == null && t.priceUsd != null && t.priceEur != null) extras.push(`€ ${t.priceEur.toFixed(2)} EUR`);
                            return extras.length > 0 ? (
                              <div className="flex gap-2.5 mt-1 text-[11px] font-semibold text-slate-500 bg-slate-50 rounded-lg py-0.5 px-1.5 w-fit">
                        ...
                        <div className="mb-4">
                          ...
                          <div className="flex items-center gap-3 mt-3">
                            <div className="bg-purple-50 rounded-xl px-3 py-2 border border-purple-100/50">
                               ...
                               <p className="font-black text-purple-900">{p.price != null ? `R$ ${p.price.toFixed(2)}` : '—'}</p>
                            </div>
                            {(p.priceUsd != null || p.priceEur != null) && (
                              <div className="flex flex-col text-[10px] font-bold text-slate-500 bg-slate-50 border border-slate-100 rounded-xl px-3 py-1 justify-center">
                                {p.priceUsd != null && <span>$ {p.priceUsd.toFixed(2)} USD</span>}
                                {p.priceEur != null && <span>€ {p.priceEur.toFixed(2)} EUR</span>}
                              </div>
                            )}
                            <div>
        ...
        {modalStep === 'form' && (
          <div className="space-y-5 pt-1 pb-2">
            ...
            <div className="grid sm:grid-cols-2 gap-5">
              ...
              />
              <Input
                label="Valor Opcional em Dólar (US$)"
                type="text"
                ...
                onBlur={(e) => {
                  const formatted = formatBRL(e.target.value);
                  setFormPriceUsd(formatted);
                }}
                size="lg"
              />
              <Input
                label="Valor Opcional em Euro (€)"
                type="text"
        ...
        {modalStep === 'package-form' && (
          <div className="space-y-5 pt-1 pb-2">
            ...
            <div className="flex flex-col gap-1.5">
               ...
               >
                 {rows.map(r => (
                   <option key={r.id} value={r.id}>{r.name}{r.price != null ? ` - R$ ${r.price.toFixed(2)}` : ''}</option>
                 ))}
               </select>
            ...
            <div className="grid sm:grid-cols-2 gap-5">
              ...
              />
              <Input
                label="Valor Total em R$ (opcional)"
                type="text"
              ...
              />
              <Input
                label="Valor Total Opcional em Dólar (US$)"
                type="text"
                ...
                onBlur={(e) => {
                  const formatted = formatBRL(e.target.value);
                  setPkgPriceUsd(formatted);
                }}
                size="lg"
              />
              <Input
                label="Valor Total Opcional em Euro (€)"
                type="text"
                placeholder="Ex: 90,00"
                value={pkgPriceEur}
                onChange={(e) => setPkgPriceEur(e.target.value)}
                onBlur={(e) => {
                  const formatted = formatBRL(e.target.value);
                  setPkgPriceEur(formatted);
                }}
                size="lg"
              />
            </div>
