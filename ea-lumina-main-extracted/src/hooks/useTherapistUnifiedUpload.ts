...
export interface TherapistUnifiedUploadOptions {
  ...
  /** Título personalizado para o certificado (opcional; se não informado usa o nome do arquivo) */
  ...
  const handleFileChange = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (type === 'certification') {
        const title = o.certificationName?.trim()
        formData.append('name', title || file.name.replace(/\.[^/.]+$/, ''))
      }
